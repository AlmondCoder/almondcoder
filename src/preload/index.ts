...
const API = {
  ...
  // Worktree methods
  createWorktree: (
projectPath: string, branch: string, promptText: string, promptId: string, undefined: undefined  ) =>
    ipcRenderer.invoke(
      'create-worktree',
      projectPath,
      branch,
  ...
  executeClaudeSDK: (
    ...
    if (onOutput) {
      const handler = (_event: any, data: { type: string; data: string }) => {
        console.log('🔌 [Preload] Received IPC message:', { type: data.type, dataLength: data.data?.length })
        onOutput(data)
      }
      ipcRenderer.on('command-output', handler)

      return ipcRenderer
        .invoke('execute-claude-sdk', options)
        .finally(() => {
   ...
   */
  onToolPermissionPending: (callback: (request: ToolPermissionRequest) => void) => {
   ...
   * LOGIC: If user doesn't respond within 30 seconds, main process sends timeout event
   */
  onToolPermissionTimeout: (callback: (data: { requestId: string }) => void) => {
